...
fn p1(seeds: Vec<usize>, categories: &Vec<Vec<(usize, usize, usize)>>) -> usize {
    seeds
        .iter()
        .map(|seed| {
            categories.clone().iter().fold(*seed, |v, cat| {
                cat.iter()
...
fn p2(seeds: Vec<usize>, categories: &Vec<Vec<(usize, usize, usize)>>) -> usize {
    let seed_ranges: Vec<(usize, usize)> = seeds.chunks(2).map(|s| (s[0], s[0] + s[1])).collect();
    categories.iter().fold(seed_ranges, |seed_ranges, cat| {
            ...
            let mut unmapped_dest = vec![seed_range.to_owned()];
                for mapping in cat {
                    ...
                    for r in unmapped_dest {
                        ...
                        if pre.0  < pre.1  { temp_range.push(pre); }
                        if over.0 < over.1 { mapped_dest.push((over.0+offset, over.1+offset)); }
                        if pos.0  < pos.1  { temp_range.push(pos); }
    ...
    }).iter().min_by_key(|(i,_)| *i).unwrap().0
}

#[aoc::puzzle("05.txt")]
#[aoc::assert("403695602", "219529182")]
