...
#[aoc::day(18, "Lavaduct Lagoon")]
#[aoc::asserts("28911", "77366737561114")]
fn main(input: String, line_ending: &str) -> (usize, usize) {
    let lines = input.split(line_ending);
    let plans = lines.clone().map(|l| {
        (
            l.as_bytes()[0],
            l[2..l.len() - 10].parse::<isize>().unwrap(),
        )
    });
...
fn calc_area(plan: Vec<(u8, isize)>) -> usize {
    ...
    let p = plan
        .iter()
        .fold((vec![(0, 0)], 0, 0), |(mut acc, mut x, mut y), (dir, n)| {
            match dir {
                b'U' => y += n,
                b'D' => y -= n,
                b'L' => x -= n,
    ...
    let area = (1..p.len())
        .fold(0, |sum, i| {
            sum + p[i].0 * (p[(i + 1) % p.len()].1 - p[i - 1].1)
        })
        .abs()
        / 2;
