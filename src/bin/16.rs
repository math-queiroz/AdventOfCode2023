...
#[aoc::day(16, "The Floor Will Be Lava")]
#[aoc::asserts("6883", "7228")]
fn main(input: String, line_ending: &str) -> (usize, usize) {
    ...
    let corners = (0..h).map(|i| ((0, i), (1, 0))).chain(
        (0..h).map(|i| ((w - 1, i), (-1, 0))).chain(
            (0..w)
                .map(|i| ((i, 0), (0, 1)))
                .chain((0..w).map(|i| ((i, h - 1), (0, -1)))),
        ),
    );
    (
        trace_from(&grid, (0, 0), (1, 0)),
        corners
            .map(|(pos, dir)| trace_from(&grid, pos, dir))
            .max()
            .unwrap(),
    )
}
