...
#[aoc::day(10, "Pipe Maze")]
#[aoc::asserts("7173", "291")]
fn main(input: String, line_ending: &str) -> (isize, isize) {
    let connections = PIPES.into_iter().collect::<HashMap<_, _>>();
    ...
    let s_pos = pipes
        ...
        .find_map(|(y, l)| {
            l.iter()
                .enumerate()
                .find_map(|(x, v)| if *v == b'S' { Some((x, y)) } else { None })
        })
        .unwrap();
    ...
    loop {
        ...
        let next = NEIGHBOURS
            ...
            .find_map(|n| {
                let p = (
                    (pos.0 as isize + n.0 .0) as usize,
                    (pos.1 as isize + n.0 .1) as usize,
                );
                if p == prev_pos {
                    return None;
                };
                let Some(pipe) = pipes.get(p.1).and_then(|l| l.get(p.0)) else {
                    return None;
                };
                let conn1 = connections
                    .get(pipes.get(pos.1).and_then(|l| l.get(pos.0)).unwrap())
                    .unwrap();
                let conn2 = connections.get(pipe).unwrap();
                if conn1 & n.1 .0 > 0 && conn2 & n.1 .1 > 0 {
                    Some(p)
                } else {
                    None
                }
    ...
    pipes[s_pos.1][s_pos.0] = match (
        "|F7".as_bytes().contains(
            pipes
                .get(s_pos.1 - 1)
                .and_then(|l| l.get(s_pos.0))
                .unwrap_or(&b'S'),
        ),
        "|LJ".as_bytes().contains(
            pipes
                .get(s_pos.1 + 1)
                .and_then(|l| l.get(s_pos.0))
                .unwrap_or(&b'S'),
    ...
    let ii = pipes.clone()[min_coord.1..max_coord.1 + 1]
        .iter()
        .enumerate()
        .fold(0, |count, (y, l)| {
