...
#[aoc::day(03, "Gear Ratios")]
#[aoc::asserts("544664", "84495585")]
        ...
        while x < line.len() {
            ...
            while x < line.len() && line[x].is_ascii_digit() {
                for n in &[
                    ...
                    let Some(&n_char) = lines.get(pos.1).and_then(|l| l.get(pos.0)) else {
                        continue;
                    };
            ...
            if x > num_start_index {
                if let Some(symbol) = symbol {
                    let number = line[num_start_index..x]
                        .iter()
                        .fold(0, |a, n| a * 10 + (n - b'0') as usize);
