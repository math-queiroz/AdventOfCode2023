#[aoc::day(13, "Point of Incidence")]
#[aoc::asserts("34100", "33106")]
...
fn find_mirror(rows: &Vec<Vec<u8>>, limit: usize) -> Option<usize> {
    for i in 1..rows.len() {
        ...
        let diff = h1.iter().zip(h2.iter()).fold(0, |diff, (a1, a2)| {
            diff + a1
                .iter()
                .zip(a2.iter())
                .fold(0, |d, (v1, v2)| d + (v1 != v2) as usize)
        });
        if diff == limit {
            return Some(i);
        }
...
fn cols_from_rows(rows: &[Vec<u8>]) -> Vec<Vec<u8>> {
