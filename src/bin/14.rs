...
#[aoc::day(14, "Parabolic Reflector Dish")]
#[aoc::asserts("113424", "96003")]
fn main(input: String, line_ending: &str) -> (usize, usize) {
    ...
    for i in 1..CYCLES {
        ...
        if let Some(cfg_i) = cfgs.insert(grid.clone(), i) {
            if (CYCLES - i) % (i - cfg_i) == 0 {
                break;
            };
        }
    }
    (
        eval_north_stress(&mut p1_grid),
        eval_north_stress(&mut grid),
    )
}

fn move_all(grid: &mut [Vec<u8>], stones: &mut [(usize, usize)]) {
    let mut moving = true;
    while moving {
        moving = false;
        for (i, (x, y)) in stones.to_owned().iter().enumerate() {
            if grid.get(*y - 1).map(|row| row[*x]) == Some(b'.') {
                moving = true;
                grid[*y][*x] = b'.';
                grid[*y - 1][*x] = b'O';
                stones[i] = (*x, *y - 1);
...
fn rotated_grid(grid: &mut Vec<Vec<u8>>, stones: &mut Vec<(usize, usize)>) {
    ...
    (0..grid.len()).for_each(|y| {
        (0..grid[0].len()).for_each(|x| rotated[x][(grid[0].len() - 1) - y] = grid[y][x])
    });
    let translated = stones
        .iter()
        .map(|c| ((grid[0].len() - 1) - c.1, c.0))
        .collect::<Vec<_>>();
    (*grid, *stones) = (rotated, translated);
}

fn eval_north_stress(grid: &mut [Vec<u8>]) -> usize {
    grid.iter().rev().enumerate().fold(0, |l_acc, (row, l)| {
        l_acc
            + l.iter()
                .fold(0, |c_acc, c| c_acc + (row + 1) * (*c == b'O') as usize)
    })
}
